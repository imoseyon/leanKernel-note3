...
static int first_sleep_wakeup = 1;
...
static struct dsi_cmd get_elvss_control_set(int cd_idx)
{
	...
	if(msd.dstat.acl_on || msd.dstat.siop_status) {
		...
		elvss_common_cmds_list.cmd_desc[1].payload[1] = 0x4C;
		...
		if (msd.dstat.temperature > 0) {
			...
			elvss_common_cmds_list.cmd_desc[1].payload[2] =
				smart_elvss_zero_cmds_list.cmd_desc[cmd_idx].payload[2];
		} else {
			elvss_common_cmds_list.cmd_desc[1].payload[2] = 0x8A;
		}

		pr_debug("ELVSS for SMART_ACL cd_idx=%d, cmd_idx=%d\n", cd_idx, cmd_idx);
	} else {

		elvss_common_cmds_list.cmd_desc[1].payload[1] = 0x5C;
		...
		if (msd.dstat.temperature > 0) {
			...
			elvss_common_cmds_list.cmd_desc[1].payload[2] =
				smart_acl_zero_cmds_list.cmd_desc[cmd_idx].payload[2];
		} else {
			elvss_common_cmds_list.cmd_desc[1].payload[2] = 0x8A;
		}
...
static struct dsi_cmd get_elvss_lowtemp_control_set(int cd_idx)
{
	...
	if(msd.dstat.acl_on || msd.dstat.siop_status) {
		elvss_lowtemp_cmds_list.cmd_desc[1].payload[1] =
			smart_elvss_lowtemp_cmds_list.cmd_desc[cmd_idx].payload[1];
	} else {
		elvss_lowtemp_cmds_list.cmd_desc[1].payload[1] =
			smart_acl_lowtemp_cmds_list.cmd_desc[cmd_idx].payload[1];
	}
...
int samsung_switching_lcd(int flip_hall)
{
	...
	pr_info("%s ++ flip(%d) poweroff_charging(%d)\n", __func__, flip_hall, poweroff_charging);

	if (first_sleep_wakeup && !poweroff_charging) {
		msd.dstat.lcd_sel = flip;
					...
					struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_samsung_parse_panel_cmd(np, &smart_elvss_zero_cmds_list,
				"samsung,panel-smart-elvss-zero-cmds-list");
	...
	mdss_samsung_parse_panel_cmd(np, &smart_acl_zero_cmds_list,
				"samsung,panel-smart-acl-zero-cmds-list");
