				       ...
				       struct mdss_mdp_pipe **ppipe)
{
	...
	if (ret) {
		pr_err("..lm..unable to satisfy performance. ret=%d\n", ret);
		goto exit_fail;
	}

	ret = __mdss_mdp_overlay_setup_scaling(pipe);
	if (ret) {
		pr_err("..lm.unable to seutp ovl scaling. ret=%d\n", ret);
		goto exit_fail;
	 }

	ret = mdss_mdp_smp_reserve(pipe);
	if (ret) {
		pr_err("..lm..mdss_mdp_smp_reserve failed. ret=%d\n", ret);
		goto exit_fail;
	}
...
exit_fail:
	pr_err("..lm..exit_fail, ret=%d\n",ret);
	mdss_mdp_pipe_unmap(pipe);

	mutex_lock(&mfd->lock);
	if (pipe->play_cnt == 0) {
		pr_err("..lm..failed for pipe %d\n", pipe->num);
	...
	list_for_each_entry(pipe, &mdp5_data->pipes_used, used_list) {
		pr_err("..lm..freeing allocations for pipe %d\n", pipe->num);
