...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		...
		rmi4_data->current_page = MASK_8BIT;            //  added Page masking routine for ghost touch
		...
		const struct i2c_device_id *dev_id)
{
	...
	printk(KERN_ERR "%s  touch_fpcb_version =0x%x",__func__,touch_fpcb_version);
