			       ...
			       struct cpr_regulator *cpr_vreg)
{
	...
	rc = of_property_read_u32_array(of_node, "qti,pvs-voltage-table",
				...
				struct device_node *of_node)
{
	...
	rc = of_property_read_u32_array(of_node,
			"qti,pvs-version-fuse-sel", fuse_sel, 4);
	if (!rc) {
					...
					struct device *dev)
{
	...
	if (!corners_mapped) {
		...
		if (rc) {
			pr_err("qti,cpr-corner-map missing, rc = %d\n", rc);
			return rc;
		}
	}

	prop = of_find_property(dev->of_node,
			"qti,cpr-speed-bin-max-corners", NULL);
	if (!prop) {
		cpr_debug("qti,cpr-speed-bin-max-corner missing\n");
		return 0;
	}
	...
	rc = of_property_read_u32_array(dev->of_node,
		"qti,cpr-speed-bin-max-corners", tmp, size);
	if (rc < 0) {
		kfree(tmp);
	 ...
	 * qti,cpr-speed-bin-max-corners property are incorrect,
	...
	prop = of_find_property(dev->of_node,
			"qti,cpr-corner-frequency-map", NULL);
	if (!prop) {
		cpr_debug("qti,cpr-corner-frequency-map missing\n");
		return 0;
	}
	...
	rc = of_property_read_u32_array(dev->of_node,
		"qti,cpr-corner-frequency-map", tmp, size);
	if (rc < 0) {
	...
	rc = of_property_read_u32(dev->of_node,
		"qti,cpr-quot-adjust-scaling-factor-max",
		&max_factor);
	if (rc < 0) {
				     ...
				     struct cpr_regulator *cpr_vreg)
{
	...
	rc = of_property_read_u32_array(of_node, "qti,cpr-quotient-adjustment",
					...
					struct cpr_regulator *cpr_vreg)
{
	...
	rc = of_property_read_u32_array(of_node, "qti,cpr-voltage-ceiling",
		&cpr_vreg->ceiling_volt[CPR_FUSE_CORNER_SVS],
	...
	rc = of_property_read_u32_array(of_node, "qti,cpr-voltage-floor",
		&cpr_vreg->floor_volt[CPR_FUSE_CORNER_SVS],
		CPR_FUSE_CORNER_MAX - 1);
	if (rc < 0) {
