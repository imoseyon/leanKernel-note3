...
void msm_bus_bimc_use_thresh(struct msm_bus_fabric_registration *fab_pdata,
	struct msm_bus_inode_info *info)
{
	struct msm_bus_bimc_info *binfo;
	bool req;
	...
	binfo = (struct msm_bus_bimc_info *)fab_pdata->hw_data;
	if (info->thresh_flag) {
		MSM_BUS_DBG("AXI: Node: %d, Threshold flag set. Enable BKE\n",
			info->node_info->id);
		req = BKE_ON;

	} else {
		req = BKE_OFF;
		MSM_BUS_DBG("AXI: Node: %d, Threshold flag reset. Disable BKE\n",
			info->node_info->id);
	}
	bke_switch(binfo->base, info->node_info->qport[0], req);
}
	...
	int64_t add_bw)
{
	...
	MSM_BUS_DBG("BIMC: Update bw for ID %d, hop: %d: %lld\n",
		info->node_info->id, hop->node_info->id, add_bw);
